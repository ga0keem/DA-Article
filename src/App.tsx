    ...
    const handleScroll = () => {
      ...
      if (window.innerWidth <= 1024) {
        ...
        textStickies.forEach((textSticky, textIndex) => {
          ...
          // 그룹의 첫 번째와 마지막 섹션의 rect 가져오기
          const firstSectionIndex = groupSections[0];
          const lastSectionIndex = groupSections[groupSections.length - 1];

          const groupStartRect = sections[firstSectionIndex].getBoundingClientRect();
          const groupEndRect = sections[lastSectionIndex].getBoundingClientRect();

          // 그룹의 첫 번째 섹션 상단이 화면 하단 진입 전이면 숨김
          if (groupStartRect.top > windowHeight || groupEndRect.bottom < 0) {
            textSticky.style.opacity = '0';
            textSticky.style.visibility = 'hidden';
            return;
          }

          // 진행도 계산 - 첫 번째 섹션이 화면에 들어올 때 바로 시작
          // groupStartRect.top이 windowHeight일 때 progress가 작은 양수가 되도록 조정
          const groupHeight = groupEndRect.bottom - groupStartRect.top;
          const adjustedProgress = (windowHeight - groupStartRect.top + windowHeight * 0.1);
          const progress = Math.max(0, Math.min(1,
            adjustedProgress / (windowHeight + groupHeight)
          ));

          // top 위치 계산
          const startPosition = groupStartRect.bottom - 100;
          const moveDistance = windowHeight * progress * 1.0;
          const currentTop = startPosition - moveDistance;

          // opacity 계산 - 섹션 진입 시 바로 보임
          let opacity = 1;
          if (progress > 0.85) {
            opacity = Math.max(0, (1 - progress) / 0.15);
          }

          textSticky.style.top = `${currentTop}px`;
          textSticky.style.opacity = `${opacity}`;
          textSticky.style.visibility = 'visible';
        });
      } else {
        // 데스크톱에서는 화면 중앙에 있는 섹션의 텍스트만 표시
        // 각 텍스트 박스마다 한 번씩만 처리
        textStickies.forEach((textSticky, textIndex) => {
          if (!textSticky) return;
          ...
          if (firstRect.top < windowHeight / 2 && lastRect.bottom > windowHeight / 2) {
        ...
        <div className="content-wrapper">
          <div className="chart-column">
            ...
            <div className="summary-desc">
              한국 대학가의 유학생 수 증가는 단순한 숫자 증가를 넘어 대학과 지역사회, 나아가 국가 경제에까지 큰 변화를 가져오고 있습니다. 유학생이 많아지면서 글로벌 경쟁력은 강화되고 있지만, 언어와 문화의 벽은 새로운 도전 과제가 되고 있습니다.<br />
              대학가의 다문화화는 앞으로 어떤 모습이어야 할지, 우리 모두 함께 고민해 볼 때입니다.
            </div>
          ...
          <aside className="text-column">
            ...
            <div className="text-sticky">
              <h3>유학생 수, 2년 새 40% 이상 증가.. 대학가가 빠르게 국제화</h3>
            ...
            <div className="text-sticky">
              ...
              <p>한국인 학생 A씨와 중국인 유학생 B씨는 대학생활 중 다가가기 어려운 거리감을 느낀다고 말합니다. A씨는 “수업에서는 자주 마주치지만, 팀 프로젝트 외에는 개인적으로 연락하거나 이야기를 나눌 기회가 거의 없습니다”라고 전했습니다. 반면 B씨는 “유학생들끼리 교류하는 경우가 더 많고, 한국 학생들과는 교류가 적습니다”라고 털어놓았습니다. 특히 팀 프로젝트에서는 언어 차이가 큰 장애물이 되고 있습니다. A씨는 “말하는 속도와 단어 수준이 달라서 설명을 몇 번이나 반복해야 할 때가 많고, 서로 이해가 맞지 않아 곤란할 때가 많습니다”라고 전했습니다. B씨도 “한국어가 충분하지 않아 이해하지 못하는 부분이 자주 발생합니다”라고 어려움을 토로했습니다. 게다가 일부 한국 학생들은 언어 문제로 인해 유학생과 함께 팀 프로젝트를 하는 것을 꺼리는 경향도 있다고 합니다. 긍정적인 사례도 있습니다. 한국인 A씨는 언어 능력이 뛰어난 유학생들은 팀 프로젝트 참여도가 높고, 빠른 이해력에 깊은 인상을 받았다고 말했습니다. 이처럼 양측 모두 협력과 소통의 가치를 느끼고 있으나, 언어 격차가 때때로 그 과정을 어렵게 만들기도 합니다.</p>
            </div>
            <div className="text-sticky">
